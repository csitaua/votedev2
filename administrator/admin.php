...
$rs_all = $my->query("select count(*) as total_all from users");
$rs_active = $my->query("select count(*) as total_active from users where approved='1'");
$rs_total_pending = $my->query("select count(*) as tot from users where approved='0'");

$temp = $rs_total_pending->fetch_assoc();
$total_pending = $temp['tot'];
$temp = $rs_all->fetch_assoc();
$all = $temp['total_all'];
$temp=$rs_active->fetch_assoc();
$active = $temp['total_active'];
...
?>
    ...
    <td width="74%" valign="top" style="padding: 10px;"><h2><font color="#FF0000">Administration
	  ...
	  $rs_total = $my->query($sql) or die(mysql_error());
	  ...
	  $rs_results = $my->query($sql . " limit $start,$page_limit") or die(mysql_error());
	  ...
	  // outputting the pages
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
          ...
          <?php while ($rrows = $rs_results->fetch_assoc()) {?>
          <tr>
...
{
$rs_dup = $my->query("select count(*) as total from users where user_name='$post[user_name]' OR user_email='$post[user_email]'") or die(mysql_error());

$temp = $rs_dup->fetch_assoc();
list($dups) = $temp['total'];
...
$my->query("INSERT INTO users (`user_name`,`user_email`,`pwd`,`approved`,`date`,`user_level`)
